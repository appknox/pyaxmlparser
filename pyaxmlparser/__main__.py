...
try:
    from .__init__ import __version__, __title__, __package_name__, APK, AXMLPrinter
    from . import utils
...
def get_files(args):
    ...
    if args.path:
        for item_path in args.path:
            if (
                isinstance(item_path, (bytes, str))
                and item_path
...
def get_info(args):
    ...
    if check_files:
        ...
        print(
            "Error. path for check APK - {} not found".format(args.path),
            file=sys.stderr,
        )
        exit(1)
...
def get_xml(args):
    output = None
    if (
        isinstance(args.output, (bytes, str))
        and args.output
        and os.path.exists(os.path.dirname(args.output))
    ):
        output = args.output
    check_files = get_files(args)
    if check_files:
        ...
        print(
            "Error. path for output xml - {} not found".format(args.path),
            file=sys.stderr,
        )
        exit(1)
...
def get_parser():
    ...
    parser.add_argument(
        "path", nargs="+", type=str, default=None, help="path for file or dir."
    )

    subparsers = parser.add_subparsers(help="List of commands")

    info_parser = subparsers.add_parser("info", help="output info from file")
    info_parser.set_defaults(func=get_info)

    xml_parser = subparsers.add_parser("xml", help="output xml from file")
...
def find_options(help_text=None, choice=None):
    ...
    :return:
    """
    if not isinstance(help_text, (bytes, str)):
        help_text = ""
    if not isinstance(choice, (bytes, str)):
        choice = "unknown"
    new_list = []
